...
DEV = './data/dev.data'
MODEL = './src/tmp/depModel3/model.ckpt'
CHECK = './src/tmp/depModel3/checkpoint'
GRPAH = './src/tmp/depModel3/model.ckpt.meta'
...
class depModel():
	...
	def __init__(self):
		...
		self.x, self.y = self.modelTrained()
		self.i = 0
	...
	def embeddingLayer(self, word_inputs, pos_inputs, labels_inputs, word_embed_dim = 128, pos_embed_dim = 64, labels_embed_dim = 64):
	...
	def hiddenLayer_1(self, x, hid_dim = 800):
	...
	def hiddenLayer_2(self, x, hid_dim = 800):
	...
	def outputLayer(self, x):
		...
		return tf.nn.softmax(tf.matmul(x, w) + b, name='outputLayer/predict_y') # _, 93
	
	def modelGraph(self, x_size=52):

		# define place holder
		actions_dim = len(self.actions)
		x = tf.placeholder(tf.int32, shape=[None, x_size], name='input_x') # _, 52
		y_ = tf.placeholder(tf.float32, shape=[None, actions_dim], name='label_y') # _, 93
	...
	def trainer(self, minibatch_size=512):
	...
	def modelTrained(self):
		...
		# restore variables
		saver = tf.train.import_meta_graph(GRPAH)
		saver.restore(self.sess, MODEL)
		graph = tf.get_default_graph()
		x = graph.get_tensor_by_name('input_x:0')
		y = graph.get_tensor_by_name('outputLayer/predict_y:0')
	...
	def decode(self, sample):
		...
		# decode
		best_act = self.sess.run(tf.argmax(self.y, 1), feed_dict = {self.x: sample})[0]
